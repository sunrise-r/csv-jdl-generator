...
public class I18NGenerator {
    ...
    public void generate(String appName, File resourceFolder, Map<String, List<RawData>> rawData, List<JdlEntity> entities) {
        List<I18nModel> i18nModel = i18NService.generateModel(appName, rawData, entities);
        i18nModel.stream().forEach(model -> i18NService.saveModel(resourceFolder, model));
    }

    public void generate(JdlConfig jdlConfig, Map<String, List<RawData>> rawData, List<JdlEntity> entities) {
        String appName = jdlConfig.getGateway();
        File resourceFolder = Paths.get(jdlConfig.getTargetResourceFolder()).toFile();
        this.generate(appName, resourceFolder, rawData, entities);
